...
class AISpleenNIFTInnUNetSegApp(Application):
    ...
    def compose(self):
        ...
        if "_" in Path(app_input_path).name:
            output_file = Path(app_output_path).joinpath(str(Path(output_file).name).split("_")[0] + ".nii.gz")
        if Path(output_file).is_file():
            output_file = Path(app_output_path).joinpath(Path(app_input_path).name.replace(".nii.gz", "_seg.nii.gz"))
            if "_" in Path(app_input_path).name:
                output_file = Path(app_output_path).joinpath(str(Path(output_file).name).split("_")[0] + "_seg.nii.gz")
