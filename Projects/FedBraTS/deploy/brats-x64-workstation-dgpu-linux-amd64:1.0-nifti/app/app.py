...
class AIBraTSNIFTIMONetSegApp(Application):
    ...
    def compose(self):
        ...
        T1C_nifti_loader_op = NiftiDataLoader(
            self, CountCondition(self, 1), input_path=app_input_path, name="t1c_nifti_loader_op", modality_mapping="-t1c.nii.gz"
        )

        T1N_nifti_loader_op = NiftiDataLoader(
            self, CountCondition(self, 1), input_path=app_input_path, name="t1n_nifti_loader_op", modality_mapping="-t1n.nii.gz"
        )
        T2F_nifti_loader_op = NiftiDataLoader(
            self, CountCondition(self, 1), input_path=app_input_path, name="t2f_nifti_loader_op", modality_mapping="-t2f.nii.gz"
        )
        T2W_nifti_loader_op = NiftiDataLoader(
            self, CountCondition(self, 1), input_path=app_input_path, name="t2w_nifti_loader_op", modality_mapping="-t2w.nii.gz"
        )
        ...
        bundle_spleen_seg_op = MONetBundleInferenceOperator(
            self,
            input_mapping=[IOMapping("T1C", Image, IOType.IN_MEMORY), IOMapping("T1N", Image, IOType.IN_MEMORY), IOMapping("T2F", Image, IOType.IN_MEMORY), IOMapping("T2W", Image, IOType.IN_MEMORY)],
        ...
        for file in files:
            if file.name.endswith("-t2f.nii.gz"):
                id_prefix = file.name.replace("-t2f.nii.gz", "")
        ...
        self.add_flow(
            T1C_nifti_loader_op, bundle_spleen_seg_op, {("image", "T1C")}
        )
        self.add_flow(
            T1N_nifti_loader_op, bundle_spleen_seg_op, {("image", "T1N")}
        )
        self.add_flow(
            T2F_nifti_loader_op, bundle_spleen_seg_op, {("image", "T2F")}
        )
        self.add_flow(
            T2W_nifti_loader_op, bundle_spleen_seg_op, {("image", "T2W")}
        )
